	...
	*/
	...
	getChapters: function() {
		...
		for(var indS = 0; indS < sL; indS++)
		{
			...
			if (curSlide.Chapter !== null && $.trim(curSlide.Chapter).length > 0) chapters.push({ "name" : curSlide.Chapter, "slide" : indS });
		}
	...
	getCurrentChapter: function(slide) {
		...
		{
			var curSlide = this._slides[indS];
			if (curSlide.Chapter !== null && $.trim(curSlide.Chapter).length > 0) { currChapter++; }
			if (indS == slide) break;
		}
		return currChapter;
	...
	slideCount: function() { return this._slides.length; },

	detachedSlideCount : function() { return this._detachedSlides.length; },
...
var Slideshow_Group = $.inherit({
		...
		if(this._options.anim["compositeItems"] && this._options.anim["compositeItems"].length > 0){
			...
			if(animationType != 1){
				...
				if(this._options.anim.dir !="-1" && this._options.anim.dir !="" ){
					this.item.animateCSS(this._options.anim.dir,{duration:parseInt(animDuration),animationClass:"",callback: $.proxy(function() {
				...
				$.each(this._options.anim["compositeItems"], $.proxy(function(key,  underElements){
					if($.inArray(underElements.id, this.animtedItemIds) > -1) this.item.find("#animateItemText" + underElements.id).animateCSS(underElements.dir,{duration:animDuration,animationClass:"",callback:
					$.proxy(function() {
	...
	playNext: function(options){
		...
		$.each(this._options.anim["compositeItems"], $.proxy(function(key,  underElements){
			if(underElements.id == nextAnim) this.item.find("#animateItemText" + underElements.id).animateCSS(underElements.dir,{duration:parseInt(options.animDur),animationClass:"",callback:
			$.proxy(function() { 
				this.playAnimDuring(underElements);
...
var SlideShow_Picture = $.inherit({
	...
	imageFound: function() { return this._pictureComplete; },
	imageHDFound : function() { return this._pictureHDComplete; },
	...
	getPicture: function() { return (this._pictureHDComplete) ? this._pictureHD : this._picture; }
...
var SlideShow_Slide = $.inherit({
	...
	getReader: function() { return this._reader; },
	    	    ...
	    	    if(animation["compositeType"] == 19 && animation["componentItem"]){
                    ...
                    ePublisherComponent.loadComponent({
                      // 'container' : $('#animateItemText' + animation.id),
				       'component': { 'id': component.id, 'name': component.name, 'location': component.location, 'file': component.file, 'object': component.object},
	...
	hasAnimation: function() { return (this.Animation !== null); },
	hasNote: function() { return (this.Notes !== null && this.Notes.length > 0); },
	hasQuizz: function() { return (this.Quizz instanceof SlideShow_Quizz); },
	...
	countImages: function() { return this._pictures.length;	}
...
var SlideShow_Animation = $.inherit({
	...
	__constructor : function($xml, slide, reader) {
		...
		this._reader = reader;
		var refAnim = this;
		this.IS_IPAD = (/iPad|iPhone|iPod/.test(navigator.platform) ||
		(navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1)) &&
		!window.MSStream ;

		this.clickEvent = IS_IPAD ? "touchend" : "click";
	
		$xml.find("item").each(function() {
		
			var $this = $(this);
			var anim = { 'id': $this.attr("id"), 'x': parseFloat($this.attr("x")), 'y': parseFloat($this.attr("y")), 'width': parseFloat($this.attr("width")), 'effect': $this.attr("effect"), 'dir': $this.attr("dir"), 'animOut':$this.attr("animOut"), 'duration': $this.attr("duration"), 'class': $.trim($this.attr("class")), 'style': $.trim($this.attr("style")) };
           ...
           if ($.isNumeric($this.attr("height"))) anim['height']=parseFloat($this.attr("height"));
            ...
            if($this.find("compositeItems").length > 0){
                ...
                $this.find("compositeItems").find("underItem").each(function() {
                    ...
                    var varAnim = { 'id': $currentAnim.attr("id"), 'x': parseFloat($currentAnim.attr("x")), 'y': parseFloat($currentAnim.attr("y")), 'width': parseFloat($currentAnim.attr("width")), 'effect': $currentAnim.attr("effect"), 'dir': $currentAnim.attr("dir"), 'animOut':$currentAnim.attr("animOut"), 'duration': $currentAnim.attr("duration"), 'class': $.trim($currentAnim.attr("class")), 'style': $.trim($currentAnim.attr("style")) };
                ...
                // randomise elements
                if(anim["compositeType"] == 1) anim["compositeItems"].sort(function() { return 0.5 - Math.random() });
            }
            ...
            if($this.find("componentItem").length > 0){
                ...
                if($config.find("languages").length > 0){
            	    $config.find("languages").find("language").each(function(){
            		   var $currentLg = $(this);
                       anim["componentItem"]["config"]["languages"].push({"id" : $currentLg.attr("id"), "name" : $currentLg.text()}); 
            	    });
                }
	...
	resize: function(param) {
		...
		for(var iAnim = 0, maxAnim = ((this._currentAnim + 1 > this._animation.length) ? this._currentAnim : this._currentAnim + 1) ; iAnim < maxAnim; iAnim++) {
			...
			var css = { 'top' : (picPos.top + (curAnim.y * param.ratio)) + 'px', 'left' : (picPos.left + (curAnim.x * param.ratio)) + 'px', 
           ...
           if (curAnim.height != '') css['height']= (curAnim.height * param.ratio) + 'px';
			...
			if(initCss){
				...
				if(initCss.padding){
				   ...
				   newCss = { "padding-top" : parseInt(initCss.padding[0]) * param.ratio +'px', "padding-bottom" :parseInt(initCss.padding[2]) * param.ratio +'px',
				   "padding-left" :  parseInt(initCss.padding[3]) * param.ratio +'px',  "padding-right" : parseInt(initCss.padding[1]) * param.ratio +'px'};
			...
			if(curAnim["compositeItems"]){
                ...
                var underCss = {'top' : underItem.y * param.ratio, 'left' : underItem.x * param.ratio, 'width' : underItem.width * param.ratio, 'height': underItem.height * param.ratio };
	...
	saveState: function(anim, options){
	   ...
	   if(options.length > 0){
	   	 for (var i = 0; i < options.length; i++) {anim.state[options[i]["name"]] = options[i]["value"];}
	   }
    },
    ...
    deleteState: function(anim){
    	...
    	if(anim.compositeItems && anim.compositeItems.length > 0){
    		$.each(anim.compositeItems, function(key, underItem){ if(underItem.state) delete underItem.state;})
    	}
	...
	getScore: function(){
      ...
      var score ="??%";
      if(curAnim != null){
      	var quizIds = (curAnim["linkids"] && curAnim["linkids"].trim() != "") ? curAnim["linkids"].split(";") : [];
	...
	createUnderElements: function(animation,param){
        ...
        switch(animation["compositeType"]){
			...
			if(!notRandomCh) compositeItems.sort(function(a,b){ return 0.5 - Math.random();});
             ...
             content += ' </div>';
          	...
          	case "16":
          	 ...
          	 var frontContent = "", backContent  = "", backAnim = null, composite = [], showBack = true, showFront = true;
          	 ...
          	 /* Front Face content */
	  	       $.each(animation["compositeItems"],function(key, underItem){
                        ...
                        animation.sound = (dbfaceType == "scoreMsg" && showBack) ? backAnim.sound : animation.sound;
		  	       	}

	                else if(underItem["compositeType"] == 2003){ composite.push(underItem);}
             ...
             content += ' </div>';
          	...
          	case "3":
          	 ...
          	 animation["compositeItems"].sort(function(a,b) { return a.order - b.order; } );
	         $.each(animation["compositeItems"],function(key, underItem){
            ...
            content += '<div class="tabeOptions_'+animation["id"]+' tableOptions">';
             ...
             $.each(animation["compositeItems"],function(key, underItem){
            		content   += ' <tr style="' +underItem["style"]+ '" class="optionLine"><td >'+underItem["text"]+' </td>';
            	    if(animation["options"]){
            	    	$.each(animation["options"], function(key, opt){
                          ...
                          if(isCheckbox){
                          	...
                          	content += '<input type="checkbox"  name="checkOption'+underItem['id']+'" class="checkOption" data-correct = "'+underItem["option"]+'" data-option = "'+opt.id+'" value="'+underItem.option+'"  />';
                          	content += '  <span class="checkmark"></span>';
                          	content += '</label>'
                          }
                          else{
                          	content += '<input type="radio"  name="checkOption'+underItem['id']+'" class="checkOption" data-correct = "'+underItem["option"]+'" data-option = "'+opt.id+'" value="'+underItem.option+'"  />';
                          }
            	    	  content += '</td>';
            	    	});
            	    }
           ...
           content += '</table>';
            ...
            if(animation["compositeType"] == 25 || animation["compositeType"] == 12 || ($.isNumeric(underItem.interaction) ^ isReverse)){
                ...
                content += '<div  id="animateItemText' + underItem["id"] + '" data-id="'+underItem["id"]+'" class="'+ typeElInt + ' ' + droppable + ' ' + className + ' " data-animation="'+underItem.dir+'"  data-interaction="' +interaction+ '" ';
			    var uStyle = (underItem["style"]) ? underItem["style"]: "";
			   ...
			   // if (underItem["class"]) content += ' class="' + underItem["class"] + '"';
				...
				var uStyle = 'border:1px solid #999; position: absolute; width: ' + width  + '; height: ' + height  + '; top: ' + underItem.y  + 'px; left: ' + underItem.x  + 'px;';
          		...
          		var className = (underItem["class"]) ? underItem["class"] : '' ;
          		content   += '<div id="animateItemText' + underItem["id"] + '"  class="'+droppable+'" style="' + uStyle + '" data-id="'+underItem["id"]+'"   data-interaction="' +interaction+ '" >';
                content   += '   <div class="draggableIntractive '+className+' stopSwipe" style=" '+iStyle+' " data-id="'+underItem["id"]+'"   data-interaction="' +interaction+ '"';
          	...
          	case "17":
                ...
                var percentWidth = ($.isNumeric(animation["width"]) && $.isNumeric(width)) ? ((parseFloat(width)/parseFloat(animation["width"])) * 100) + '%' : '100%';
   	  	        ...
   	  	        percentWidth = ($.isNumeric(animation["width"]) && $.isNumeric(width)) ? width + 'px' : '';
   	  	        
   	  	        content += '<select class="comboType" data-id="'+animation["id"]+'" "  style="color:#222; ' +(percentWidth != '' ? 'max-width:' + percentWidth : '' ) +'">';
          	        $.each(animation["options"], function(key, option){
                        content += '<option data-info ="' + ( (option["params"]) ? option["params"] : 0 ) + '">' + option["text"] + '</option>';
                    });
                content += '</select>';
             	...
             	if(compositeItems && compositeItems.length < 0) return ;
          	    ...
          	    if(param && param.length > 0 && param.split(",").length > 0){
          	       isValidated = true;
                      ...
                      if(memorisedSlides.indexOf(paramsArray[i]) < 0) { isValidated = false; break;}
          	  	   }
          	    }
	...
	affectEvents: function(animation,  param){
        ...
        if(goToNextAnim === true){
        	...
        	$div.on(oRef.clickEvent,function(){
		...
		container.find("#animateItemText" + animation.id + " > .interactiveElt").on(oRef.clickEvent,function(key, obj){
		...
		switch(animation["compositeType"]){
			...
			case "21":
				...
				var $div  = container.find("#animateItemText" + animation.id);
				$div.find(".customCheckbox").on(oRef.clickEvent, function(e){
					e.preventDefault();
			...
			case "23":
               ...
               if($div.find(".muloptcusom").length > 0){
                  $div.find(".muloptcusom").on(oRef.clickEvent,function(){
                  ...
                  $.each(animation["compositeItems"], function(key, underItem){
                    ...
                    if(selectOptions.length > 0){
                    	$("#item" + underItem.id +" .validate").on(oRef.clickEvent,function(){
                    		var $this = $(this);
                    		...
                    		if(isCheckbox){
                                ...
                                for (var i = 0; i < isCustom; i++) {$("#item" + underItem.id +" td").removeClass("muloptcusom" + i);}
                    			$.each(selectOptions, function(key, correctOpt){
		    ...
		    case "20":
                var $div = container.find("#animateItemText" + animation.id);
                var ref = this;
                $div.find(".stepAnswer").on(oRef.clickEvent,function(){
                ...
                $(document).off(oRef.clickEvent,".imgChoose20").on(oRef.clickEvent, ".imgChoose20", function(){
                      var notSelected = [];
			...
			container.find("#animateItemText" + animation.id + " > .interactiveElt").on(oRef.clickEvent,function(key, obj){
                   ...
                   if(interactionObj.length > 0){
                       ...
                       if($("[data-interaction="+interactionId+"]").is(":hidden")){
                           opened = true;
                   	       $("[data-interaction="+interactionId+"]").animateCSS(interactionType,{duration:500,callback :function(){}});
			...
			$("#animateItem" + animation["id"] + " .draggableIntractive ").each($.proxy(function(key, choice){
                  $(choice).draggable({
                    	...
                    	//$(choice).css({'left':'1px', 'top':'0px','z-index':0});
            ...
            case "8":
                ...
                if(animation["compositeType"] == "1"){
                container.find(".sortableContainer_" + animation["id"] + " .validateSortableElement").on(oRef.clickEvent,function(){
                ...
                if(displayColor){
                   ...
                   listItems.sort(function(a, b){return parseInt($(a).attr("data-order")) - parseInt($(b).attr("data-order")); });
                   $.each(listItems,function(key, underItem){
                ...
                $btnNext.click(function(){$(this).hide();});
            ...
            case '16':
            	...
            	default:
	                ...
	                container.find("#animateItemText" + animation.id).on('flip:done',$.proxy(function(){
	                    ...
	                    //if(animation["isBackFace"]) container.find("#animateItemText" + animation.id +" .front").css("transfrom","");
	                    //else  container.find("#animateItemText" + animation.id +" .front").css("visibility","");
            ...
            case '3':
				container.find(".panelCustom").on("touchend", function(){$(this).trigger("click");});
            ...
            case "9": 
                ...
                var modalSelector = (isFullScreen) ? "modal-fullscreen" : "myModal";
              	if(options.length == 0) return;
              	container.find("#animateItem"+elementId).on(oRef.clickEvent,$.proxy(function(){
                        	...
                        	container.find("#checkdragdropOptions").click(function(btn){
                        		...
                        		$.each(container.find(".choiceItemMirror"),function(key, item){
                        			...
                        			if(displayColor){
                        				...
                        				if(containerOption == itemOption) $(item).find(".choiceItem").css({"background-color":"green","color":"#fefefe"});
                        			    else $(item).find(".choiceItem").css({"background-color":"red","color":"#fefefe"});                            
                        			}
                        			else{
            ...
            case "14":
                ...
                animation["isBackFace"] = false ;
               $("#animateItem" + animation["id"]).on(oRef.clickEvent,$.proxy(function(e){
	...
	affectDataStyle:function(id, param){
		...
		$.each(litsElements, function(key, innerObj){
			...
			if(!$innerObj.attr("data-style")){
                ...
                var initCss = {"fontSize" : parseFloat($innerObj.css("font-size")) * (1/ratio) + "px" , "padding" : padding, "margin" : margin};
	...
	startComponent: function(animation){
		   ...
		   if(!animation.objectComponent || !animation.objectComponent.ready){
		    	...
		    	animation.timer = setInterval($.proxy(function(){ this.startComponent(animation);},this), 100);
		    	return;
		    }
		   if(animation.timer){
		    ...
		    if(animation["splitedParameters"] && animation["splitedParameters"]["Objectives"]){
                ...
                animation.objectComponent.setData({"chartType": animation["splitedParameters"]["ChartType"] ? animation["splitedParameters"]["ChartType"] : "Spider","chartData": data});
            }
            ...
            setTimeout(function(){ $("#animateItemText" + animation.id).trigger("epublisher-resize");}, 50);
		...
		if(anim.animOut != "-1" && anim.animOut != "" && anim.animOutBefore != "" && anim.animOutBefore != "-1"){
			...
			if(anim.animOutBefore!="" && curAnim.itemRef==anim.animOutBefore ){
				if(anim.animOut != "-1" && anim.animOut != "" && anim.animOut != "simple"){
					...
					param.container.find('.ssanimate' + currentThis._slide.Index + '_' + key).animateCSS(anim.animOut,{duration:parseInt(anim.duration), callback: function(){
						param.container.find('.ssanimate' + currentThis._slide.Index + '_' + key).css({"opacity":0,"visibility":"hidden"});
			...
			$.each(anim["compositeItems"], $.proxy(function(key,  underAnim){
				if(underAnim.animOutBefore ==  curAnim.itemRef) {
					if(underAnim.animOut != "" && underAnim.animOut != "-1" && underAnim.animOut != "simple"){
							...
							param.container.find('#animateItemText' + underAnim.id).animateCSS(underAnim.animOut,{duration:parseInt(curAnim.duration), 
							callback: function(){
								param.container.find('#animateItemText' + underAnim.id).css({"opacity":0,"visibility":"hidden"});
								param.container.find('#animateItemText' + underAnim.id).remove();
							}});
					}else{
						param.container.find('#animateItemText' + underAnim.id).css({"opacity":0,"visibility":"hidden"});
			...
			if (curAnim.effect == 0) {
              ...
              curAnim.dir = (( typeof window.Modernizr !== "undefined" && window.Modernizr.cssanimations == true)) ? curAnim.dir : "right";
               if(curAnim.dir=="right" || curAnim.dir=="left" || curAnim.dir=="top" || curAnim.dir=="bottom" || curAnim.dir=="fade")
               {
				...
				$item.css({"left":(picPos.left + (curAnim.x * param.ratio)),"opacity":1,"visibility":"hidden"});
				...
				param.container.find('.ssanimate' + this._slide.Index + '_' + this._currentAnim).animateCSS(curAnim.dir,{duration:parseInt(curAnim.duration),animationClass:"",callback: $.proxy(function(fct, param) {		
...
var SlideShow_Quizz = $.inherit({
	...
	__constructor : function($xml, slide) {
		...
		$xml.find("questions > *").each(function() {
		    ...
		    question["getScore"] = function() {
				...
				$(choosenItems).each(function(key,item){
					if($.inArray(item,correctChoices) >= 0) {goodDecision++;}
				});
			...
			question["randomise"] = function(){
			   ...
			   this.choices.sort(function() { return 0.5 - Math.random(); } );
			   //Manage keepposition
			   var continuee = true; var cmpt = 0;
			...
			question["getGroups"] = function() {
				...
				for(var indC = 0; indC < lC; indC++) {
					...
					{
						if (gp[indGp] == this["choices"][indC].group) { add = false; break; }
					}
			...
			question["userResponses"] = {
				...
				removeResponse: function(id) { if (this._uResp === null) this._uResp = {}; delete this._uResp[id]; },
				removeAllResponse: function() {this.isValidated = false; this._uResp = {}; },
				responseExists: function(id) { return (this._uResp !== null && ( this._uResp[id] === true || (typeof this._uResp[id] == 'string' && this._uResp[id].trim() != ""))); },
			...
			$quizzQ.find("items > i").each(function() {
				...
				{
					if (choice.Id == answers[indC]) { choice["correct"] = true; break; }
				}
        ...
        if(nbreQuestionMax != null ){
        	refQuizz._Questions.sort(function() { return 0.5 - Math.random() });
        }
	...
	fillListQuestions: function(nbreQuestions, oldQuestions){
        ...
        this._Questions = [];
        if(oldQuestions) {for (var i = 0; i < validatedQuestionsObj.questions.length; i++) { this._Questions.push(validatedQuestionsObj.questions[i]);};}
	...
	getQuestions: function() { return this._Questions; },
	...
	getQuizzScore : function(){
		var dataCom = this._Slide._dataCom;
		return dataCom.getQuizzCalculationInformation([this._Slide.Id]);
	},
