		...
		for(var islide = 0; islide < slides.length; islide++) {
			...
			if ( !selectedQuiz || ($.inArray(slide.Id,quizIds) >= 0)  ) {
				...
				if(slide.hasQuizz()){
					...
					nbreQuestionsInScore += (!slide.Quizz.scoreCalculation  && !(options && options.notScorm) ) ? 0 : ($.isNumeric(slide.Quizz.NombreQuestionMax)) ? parseInt(slide.Quizz.NombreQuestionMax) : slide.Quizz.countQuestion();
