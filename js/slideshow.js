    ...
    initialize: function(callbackinit){
        ...
        $.ajax({
            ...
            success: function(xml) {
                ...
                oRef.initializeSlideShow(oRef);
                oRef.forcedScore = oRef._xmlReader.MasteryScore;
                console.log(oRef.forcedScore);
...
*/  hideHeader :function(timer){
    ...
    positionBarShow:function(newSizeP,oldSizeP){
       ...
       if(newSizeP!=null && oldSizeP!=null){
        ...
        if(oldSizeP.height != this.prefered_img_dim.height ||  oldSizeP.width != this.prefered_img_dim.width){
            ratioX = newSizeP.width / this.prefered_img_dim.width;
        }
        ...
        /*corrected*/
        cssScale = {"-moz-transform":"scale("+ratioX+")", "-webkit-transform":"scale("+ratioX+")" , "transform":"scale("+ratioX+")"};
        ...
        $(".headersFullScreen").css({ top: (topImg +  (15 * ratioX)  ) + 'px' });


       //position the bottom bar in the right place
       // var hightWindow = $(window).height();
       // $(".fullScreenButtomBar").css({ top: ( topImg + heightImg - 10)+'px', 'bottom': '' });
       $(".fullScreenButtomBar").css({ bottom: (heightWindow - topImg - heightImg + (10 * ratioX) ) + 'px' });

       /*corrected*/
       $("#transcript").css('font-size', (ratioX*100) + '%');
       }
...
****************************/
    ...
    f_activate: function(){
        this._buttonState.next.f_dis_activate = false;
    },
    ...
    actionNext: function(options) {
        ...
        if(this.next_in_component){
            var animation = curSlide.getComponent();
            if(animation && animation['objectComponent'] && typeof animation['objectComponent'].next === "function"){
             animation['objectComponent'].next();
             return;
            }
    ...
    actionRepeat: function() {
        ...
        //console.log(curSlide.Animation.getSuspended());
...
****************************/
    ...
    canRetryQuiz: function(){
        var curSlide   = this._xmlReader.getSlide(this._dataCom.getCurrentSlide() - 1);
        var slides = this._xmlReader.getSlides();
        var canRepeat = false;
        for(var i = curSlide.Index ? curSlide.Index : 0 ; i >= 0 ; i--){
            if(slides[i].Quizz) {
            //return  (!$.isNumeric(slides[i].Quizz.RetryMaxQuizz) || slides[i].Quizz.RetryMaxQuizz > slides[i].Quizz.RetryNbre);
            canRepeat = canRepeat || (!$.isNumeric(slides[i].Quizz.RetryMaxQuizz) || slides[i].Quizz.RetryMaxQuizz > slides[i].Quizz.RetryNbre);
            }
            else if(slides[i].getAnimationsInScore().length > 0){
                var elements_list = slides[i].getAnimationsInScore();
                for(var k = 0 ; k < elements_list.length; k++){
                    var userResponses   = elements_list[k].getUserResponses();
                    var maxRetry        = slides[i].Animation ? slides[i].Animation.getParameterByName("maxRetry", elements_list[k].id) : 100;
                    canRepeat = canRepeat || userResponses.retryNbre < maxRetry;
                }
            }
        }
        return canRepeat;
   ...
   droppableZone2 : function(obj, callbackDrop){
    ...
    displayQuiz: function() {
        ...
        if (prevquestion && (!prevquestion.userResponses.isValidated || curSlide.Quizz.RetryQuizz === true)) {
            ...
            prevquestion.userResponses.isValidated = true;
             if( this._quizProgress == 1 &&
                (!curSlide.Quizz.DisplayResponse || (curSlide.Quizz.DisplayResponse && (!this._quizWaitUserChoices || curSlide.Quizz.ForceDisplayResponse))) &&
        ...
        if (curSlide.Quizz.DisplayResponse && !curSlide.Quizz.ForceDisplayResponse) {
            if (this._quizWaitUserChoices) this._quizProgress++;
            this._quizWaitUserChoices = !this._quizWaitUserChoices;
        }
        else {
    ...
    quizzDisplayResponses: function(curSlide, question) {
        ...
        if ((curSlide.Quizz.DisplayResponse && !this._quizWaitUserChoices) ||  curSlide.Quizz.ForceDisplayResponse) {
            ...
            var modelastattempt   = curSlide.Quizz.ForceDisplayResponse;
            var modenormale       = !modelastattempt; // we use this just to make things clear

            var sCsA              = curSlide.Quizz.ShowSelectedAnsColor  == true; // show Color Selected Answer
            var hr                = !curSlide.Quizz.DisplayResponseColor == true; // hide response
            var hc                = curSlide.Quizz.HideColorResponse     == true; // hide color
            var sc                = curSlide.Quizz.ForceShowComment      === true;

            var showColorAns         = modenormale && (!sCsA && !hr && !hc) || modelastattempt && (!sCsA && !hc);
            var showColorSelectedAns = modenormale && (sCsA && !hr)  || modelastattempt && (sCsA);
            var showComment          = modenormale && (!sCsA && !hr) || modelastattempt && (!sCsA) || sc;
            var showColorComment     = modenormale && (!sCsA && !hr) || modelastattempt && (!sCsA && !hc);

            //display good & bad response with colors
            this._options.container.find(".choicetxt").each(function() {
                ...
                // }
                if(showColorAns) $this.addClass(correct ? 'correctResponse': 'wrongResponse');
                else if(showColorSelectedAns && question.userResponses.responseExists(id)) $this.addClass(correct ? 'correctResponse': 'wrongResponse'); // answer is selected
            });
           ...
           // var showComment = curSlide.Quizz.DisplayResponseColor;
            ...
            if(isQuestionComment && showComment){
                ...
                if(!isError && nbreOfAns == nbreOfCorrectAns ){
                    ...
                    if(showColorComment)  $("#choicecomment").css({"color":"green"});
                    else $("#choicecomment").css({"color":""});
                   ...
                   if(showColorComment) $("#choicecomment").css({"color":"orange"});
                   else $("#choicecomment").css({"color":""});
                   $("#choicecomment").css({"display" : "block"});
                }
                else{
                    $("#choicecomment").html(question.questionIComment);
                    if(showColorComment) $("#choicecomment").css({"color":"red"});
                    else $("#choicecomment").css({"color":""});
                    $("#choicecomment").css({"display" : "block"});
                }
            ...
            if(!isError && nbreOfAns == nbreOfCorrectAns && typeof goodAnswer.Comment !== "undefined" && showComment) {
              //correct response
                ...
                $("#choicecomment").html(goodAnswer.Comment)
                $("#choicecomment").css("display", 'block');
            }
            ...
            if(badAnswer != null && typeof badAnswer.Comment !== "undefined" && showComment ){
                ...
                $("#choicecomment").html(badAnswer.Comment);
                $("#choicecomment").css("display", 'block');
            }
    ...
    quizzDisplayRepeat: function(curSlide, question){
        ...
        if(result < this._dataCom.getMasteryScore()){
            ...
            for(var iQ = 0; iQ < quizTotal; iQ++)
            {
               var q = curSlide.Quizz.getQuestion(iQ);
                q.randomise();
                if(rightToRetry) q.userResponses.removeAllResponse();
            }
        ...
        if (this._xmlReader) {
            ...
            if (curSlide.Type == "quiz") {
                ...
                if ($("#quiz").css("display") != "none") {
                   ...
                   /* Resize of Quiz button, check images, ... */
                   $(".imgChoiceCheck").css({"width": '', 'height': ''});
                   $(".imgChoiceCheck").css("cssText", "background-size: 80% 80% !important; background-position:center !important");
                   //$(".imgChoiceCheck").css('background-size','80% 80%');
                   $("#myBtnNextQuestion").css({"padding": '', 'font-size': ''});
                   var refW        = $(".imgChoiceCheck").width();
                   var refH        = $(".imgChoiceCheck").height();
                   var refpadTop   = parseFloat($("#myBtnNextQuestion").css("padding-top"));
                   var refpadLeft  = parseFloat($("#myBtnNextQuestion").css("padding-left"));
                   var refFontSize = parseFloat($("#myBtnNextQuestion").css("font-size"));
                   $(".imgChoiceCheck").css({"width": this.ResizeCoef * refW + 'px', 'height': this.ResizeCoef * refH + 'px'});
                   $("#myBtnNextQuestion, #goToNextQuestion").css("padding", this.ResizeCoef * refpadTop + 'px '+ this.ResizeCoef * refpadLeft + 'px');
                   $("#myBtnNextQuestion, #goToNextQuestion").css("font-size", this.ResizeCoef * refFontSize + 'px');
