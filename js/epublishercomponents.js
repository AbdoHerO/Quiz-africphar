...
*******************/
    ...
    this.loadSynchronousFiles = function(file, option){
        ...
        $.getScript(option.folderPath + syncfile)
                        .done($.proxy(function(f, o) {
                           ...
                           console.log(option.folderPath + syncfile);
                        }, this, file, option))
	...
	this.activateNextInComponent = function(status) {
		if (this._options && this._options.slideshow) {
			this._options.slideshow.next_in_component = status;
		}
	};
