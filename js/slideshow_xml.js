...
var SlideShow_XML = $.inherit({
	...
	*/
    __constructor: function (xml, options) {
        ...
        this.disableBackButtons = (this.getOptions("disableBackButtons") && this.getOptions("disableBackButtons") == "1") ? true : false;
        ...
        $xml.find("item").each(function () {
            ...
            if (refAnim.getParameterByName("isInScore", anim.id) == 1) {
                ...
                anim["getAnimationScore"] = function () {
                    ...
                    $.each(responses.values, $.proxy(function (keyResp, reponse) {
                        // we can delete this condition in if
                        if (reponse.id != -1 || this.compositeType == 27) numberOfResponses++; // to be sure to calculate only elements was dropped  take the MIN (nbreDrag, nbreDrop) -> (example number of draggable element > number of droppable zone)
                    }, this));
                    ...
                    switch (this.compositeType) {
                        ...
                        case "27":
                            for (var i = 0; i < responses.values.length; i++) {
                                ...
                                score += ( dragElmnt &&
                                    ($.inArray(dropZoneId, dragElmnt.linkidsArray) > -1
                                   || dragElmnt.linkidsArray.length == 0 && dropZoneId == -1)
                                 ) ?  1 : 0;
                            }
        ...
        switch (animation["compositeType"]) {
            ...
            case "16":
                ...
                $.each(animation["compositeItems"], function (key, underItem) {
                    if (underItem["compositeType"] == 2002) {
                        ...
                        // save the init sound to not erase it;
                        animation.initSound = !animation.initSound ? animation.sound : animation.initSound;
                        // affect right sound to play
                        animation.sound = ((dbfaceType == "scoreMsg" || dbfaceType == "lastAttemptsMsg") && showBack) ? backAnim.sound : animation.initSound ;
            ...
            case '15':
                ...
                if (countOptions > 0) {
                    ...
                    var options = '<td class="optionsListContainer optionsContainerDroppable" style="width:' + (columnWidth == 0 ? widthDiv : columnWidth) +  '%;">';
                            ...
                            if (userResponses.values[i].id && userResponses.values[i].value == itemInt) {
                                ...
                                if (displayColor) {
                                    ...
                                    if (compositeType != 27) $(item).addClass(containerInt == itemInt ? "correctDrop" : "errorDrop");
                                        ...
                                        || (dragElmnt.linkidsArray.length == 0 && userResponses.values[i].id == -1 );
        ...
        switch (animation["compositeType"]) {
            ...
            case "21":
                ...
                $div.find(".customCheckbox").on("touchend click", function (e) {
                    e.preventDefault();
                    $(e.target).closest('td').removeClass('incorrectshowopt correctshowopt');
            ...
            case "26":
                ...
                $("#animateItem" + animation["id"] + " .submitUnderElement").click($.proxy(function (e) {
                    ...
                    var isStrict    = this.getParameterByName("isStric", checkboxList.id) == 1 ? true : false;
                    ...
                    if(displayColor){
                        $("#animateItem" + animation["id"] + " .checkOption").each(function(key, checkbox){
                            ...
                            if(
                                ( !isStrict &&
                                    ( ($checkbox.attr('data-correct') == 1 && $checkbox.is(":checked")) ||
                                    ($checkbox.attr('data-correct') == 0 && !$checkbox.is(":checked")) )
                                )
                                ||
                                ( isStrict && ($checkbox.attr('data-correct') == 1 && $checkbox.is(":checked")))
            ...
            case "27":
                ...
                var isReverse = this.getParameterByName("reverseDragDrop", animation.id) == 1 ? true : false;
                var isInScore = this.getParameterByName("isInScore", animation.id) == 1 ? true : false;
                var showSubmit = this.getParameterByName("showSubmit", animation.id) == 1 ? true : false;
                if (isInScore) {
                    oRef._slide._dataCom._slideshow._buttonState.next.f_dis_activate = true;
                    oRef._slide._dataCom._slideshow._buttonState.play.activate = false;
                    oRef._slide._dataCom._slideshow.raiseButtonStateChanged(oRef._slide._dataCom._slideshow._buttonState);
                ...
                $("#animateItem" + animation["id"] + " .draggableIntractive").each($.proxy(function (key, choice) {
                    $(choice).draggable({
                        ...
                        stop: function (event, ui) {
                            ...
                            var timeToShowSubmit = (showSubmit && $("#animateItem" + animation["id"] + " .underInteractiveElt:has(.draggableIntractive)").length >= 1)
                            || $("#animateItem" + animation["id"] + " .underInteractiveElt:not(:has(.draggableIntractive))").length == 0 ;
                            if (timeToShowSubmit && isInScore) {
                                oRef._slide._dataCom._slideshow._buttonState.next.f_dis_activate = false;
                ...
                $("#animateItem" + animation["id"] + " .submitUnderElement").click($.proxy(function (e) {
                    ...
                    if (!timeToShowSubmit || !isInScore) return;
                   ...
                   // var displayColor = this.getParameterByName("displayColor", dragDropOpen.id) == 1 ? true : false;
                    var displayColor = $.isNumeric(this.getParameterByName("displayColor", dragDropOpen.id)) ? this.getParameterByName("displayColor", dragDropOpen.id) : 0 ;
                    ...
                    $.each(listDraggableZone, function (key, item) {
                        ...
                        var hasRightToColors = displayColor == 1 || (displayColor == 2 && $(item).parent().attr('data-interaction') !=  -"1") ;
                        ...
                        if (hasRightToColors) {
                            ...
                            if (type != 27 )  $(item).addClass(containerInt == itemInt ? "correctDrop" : "errorDrop");
                                ...
                                isCorrect = (dragElmnt && $.inArray(containerInt, dragElmnt.linkidsArray) > -1)
                                    || (dragElmnt.linkidsArray.length == 0 && !$.isNumeric(containerInt) );
                                $(item).addClass(isCorrect ? "correctDrop" : "errorDrop");
                            }
            ...
            case "7":
                ...
                container.find(".optionSubmit").click(function () {
                    ...
                    var resourceType = that.getParameterByName("resource", elementId) ;
                    ...
                    oRef._slide.players.playeraudio.actLAP('stop');
                    if(sound && resourceType && resourceType == 1){
                        var isFullScreen = that.getParameterByName("modalType", elementId) == 1 ? true : false;
                        var modalSelector = (isFullScreen) ? "modal-fullscreen" : "myModal";
                        $("#" + modalSelector + " .modal-header").css("display", "none");
                        var html = ' <video autoplay="true" controls id="video1" style="width: 100%; height: auto; margin:0 auto; frameborder:0;">\n' +
            ...
            case "15":
                ...
                if(container.find('.optionsContainer').width() !== container.find('.optionsListContainer').width()){
                    // The width of drag and drop zones not equals
                    container.find(".choiceItemMirror").on('mousedown', function(e) {

                        $(e.target).one('mouseup', function() {
                            $(e.target).off('mousemove.dragNameSpace');
                            let w = $(e.target).closest('.optionsListContainer').css('width');
                            console.log("container", w);
                        });

                        $(e.target).one('mousemove.dragNameSpace', function() {
                            console.log("list container", $(e.target).closest('.optionsListContainer').css('width'));
                            $(e.target).parent('.choiceItemMirror ').css({'width': '60px'})
                        });
                    });
                }
                ...
                container.find(".choiceItemMirror").each($.proxy(function (key, choice) {
                    $(choice).draggable({
                        ...
                        stop: function (event, ui) {
                            ...
                            if($(choice).parent().hasClass('optionsListContainer')){
                                $(choice).css('width',$(choice).parent().css('width'));
                            }
                            else if($(choice).parent().hasClass('optionsContainer')){
                                $(choice).css('width',$(choice).parent().css('width'));
                            }
                        }
                    });
...
var SlideShow_Quizz = $.inherit({
    ...
    __constructor: function ($xml, slide) {
        ...
        this.ForceShowComment = $xml.find("parm > showComment").text() == "yes" ? true : false;
